...
export default function ModificarPerfil() {
  ...
  useEffect(() => {
    if (user) {
      setLoading(true);
      const ref = doc(db, "miembros", user.email);
  ...
  const handleGuardar = async (campo) => {
    ...
    setGuardando(true);
    try {
      await updateDoc(doc(db, "miembros", user.email), {
    ...
    <div className="mb-4">
      ...
      {editando === campo ? (
        ...
        <div className="flex gap-2 items-center">
          <span className="text-white">{perfil[campo] ?? <span className="text-gray-400">Sin datos</span>}</span>
          <button
            type="button"
            className="bg-neon text-black px-3 py-1 rounded font-bold"
